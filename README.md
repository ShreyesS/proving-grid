...
| [`backend/main.py`](./backend/main.py) | **FastAPI** — `GET /state`, `POST /run`, WebSocket `/ws` (broadcasts the live run) |
| [`frontend/`](./frontend/) | **React + Vite + Cytoscape** — live topology, scoreboard, and a streaming attacker-reasoning panel |

**Data flow:** `topology.yaml` → `twin.py` → attacker loop (LLM picks the next tool) → tools mutate modeled state → every change streams over WebSocket → React UI. Vite proxies `/state` and `/ws` to port 8000 in dev.
...
| `POST /run` | `auto` | LLM if a key is set, else scripted |
...
| Reasoning panel empty but graph moved | The WS connected after the run started — `curl` the run again with the page open |
